...
const QUICK_ACTIONS = [
  {
    ...
    href: ROUTES.modals.upload,
  },
  { label: 'AI Chat', icon: 'sparkles-outline' as const, href: ROUTES.tabs.chat },
  { label: 'Quiz', icon: 'help-circle-outline' as const, href: ROUTES.app.quiz },
  { label: 'Flashcards', icon: 'albums-outline' as const, href: ROUTES.app.flashcards },
];
...
export default function HomeScreen() {
  ...
  return (
    <Screen
      title={`Hey, ${displayName.split(' ')[0] ?? 'Learner'}`}
      subtitle="Ready for a focused study session?"
      headerRight={<Avatar name={displayName} size="md" />}
      refreshing={loading}
      onRefresh={onRefresh}
    >
      {loading ? (
        ...
        <>
          ...
          <Card
            elevated
            onPress={() => push(ROUTES.app.summary)}
